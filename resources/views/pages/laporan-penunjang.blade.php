<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <div class="container-fluid py-4">
            @php
                $totalFaktorForAverage = 0;
                foreach ($tindakanPenunjang as $tindakan) {
                    $totalWaktuForAverage = $rataRataWaktu[$tindakan->id] ?? 0;
                    if ($tindakan->kategori == 'harian') {
                        $totalWaktuForAverage *= 264;
                    } elseif ($tindakan->kategori == 'mingguan') {
                        $totalWaktuForAverage *= 52;
                    } elseif ($tindakan->kategori == 'bulanan') {
                        $totalWaktuForAverage *= 12;
                    } elseif ($tindakan->kategori == 'tahunan') {
                        $totalWaktuForAverage = $totalWaktuForAverage;
                    }
                    $faktorForAverage = $totalWaktuForAverage > 0 ? ($totalWaktuForAverage / $hospitalTime) * 100 : 0;
                    $totalFaktorForAverage += $faktorForAverage;
                }
                $totalTindakanForFaktor = count($tindakanPenunjang);
                $averageFaktor = $totalTindakanForFaktor > 0 ? $totalFaktorForAverage / $totalTindakanForFaktor : 0;
                                        ...
                                        @foreach ($tindakanPenunjang as $tindakan)
                                            ...
                                            @else
                                                @php
                                                    ...
                                                    $rataWaktu =
                                                        $tindakan->waktu > 0
                                                            ? $tindakan->waktu / $tindakan->count()
                                                            : 0;
                                                    $swl =
                                                        $rataWaktu > 0 ? $jamTersediaPerTahun / ($rataWaktu / 60) : 0;
                                                @endphp
                                                <tr>
                                                    <td class="align-middle text-md ps-5">
                                                        <strong
                                                            class="text-dark">{{ $tindakan->tindakan ?? 'Tidak Ada Data' }}</strong>
                                                    </td>
                                                    <td class="text-center pb-auto text-sm">
                                                        <strong
                                                            class="text-success">{{ $totalTindakan[$tindakan->id] ?? 0 }}</strong>
                                                    </td>
                                                    <td class="text-center limitid-height text-md">
                                                        <p class="text-md">
                                                            {{ number_format($rataRataWaktu[$tindakan->id] ?? 0, 2) }}
                                                            jam</p>
                                                    </td>
                                                </tr>
                                            @endif
                        ...
                        <div class="card-body px-0 pb-2">
                            ...
                            <div class="row">
                                <div class="col-md-12 d-flex justify-content-between mb-3">
                                    <form action="{{ route('admin.laporan.index3') }}" method="GET"
                                        class="row align-items-end justify-content-center w-100">
                                        <div class="col-12 col-md-6 ps-5">
                                            <div class="d-flex align-items-end justify-content-center gap-5">
                                                ...
                                                <div
                                                    class="col  input-group input-group-static mr-2 align-items-center">
                                ...
                                <table id="author_table" class="table align-items-center mb-0">
                                    ...
                                    <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="align-middle text-center text-sm text-danger">
                                                    @php
                                                        ...
                                                        $faktor =
                                                            $totalWaktu > 0 ? ($totalWaktu / $hospitalTime) * 100 : 0;
                                        ...
                                        @php
                                            ...
                                            $totalTindakanForFaktor = count($tindakanPenunjang);
                                            $averageFaktor =
                                                $totalTindakanForFaktor > 0
                                                    ? $totalFaktor / $totalTindakanForFaktor
                                                    : 0;
                                            $averageFaktorString = number_format($averageFaktor, 2);
                                        @endphp
                                    </tbody>
                                    <tfoot>
                                        <tr>
                                            <td colspan="4" class="text-end fw-bold text-dark">Rata-rata Faktor (%)
                                            </td>
                                            <td class="text-center text-danger text-lg">
                                                <strong>{{ $averageFaktorString }}</strong>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" class="text-end fw-bold text-dark">Allowance Factor (AF)
                                            </td>
                                            <td class="text-center text-danger text-lg">
                                                <strong>{{ number_format(1 / (1 - $averageFaktor / 100), 2) }}</strong>
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
            ...
            document.addEventListener("DOMContentLoaded", function() {
                const table = $('#author_table').DataTable({
                    ...
                    columnDefs: [{
                            targets: 0,
                            orderable: false
                        } // Kolom pertama tidak bisa diurutkan
                    ],
